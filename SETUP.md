...
> `git clone`, `git`, Python или `uv`. Единственное условие -- Docker,
> проверяется на шаге 1. Node.js может дополнительно понадобиться на шаге
> 7, но только если MCP-клиент человека -- Claude Desktop.
...
На Windows Docker Desktop может дополнительно потребовать включить
виртуализацию в BIOS/UEFI и установить WSL2 при первом запуске -- Docker
Desktop сам покажет, чего именно не хватает, и как правило может сам
поставить недостающее (WSL2) после подтверждения. Дождись, пока Docker
Desktop полностью запустится (иконка в трее перестанет показывать
"Starting"), и только после этого повторяй команды выше.
...
поднят и слушает порт. Не переходи к шагу 8, пока не ответят оба --
шагу 7 (конфиг клиента) это не требуется, но шагу 8 (OAuth) требуется.
...
## Шаг 7 -- Настройка конфига MCP-клиента

**Сделай этот шаг до OAuth-авторизации (шаг 8), а не после** -- клиент
не покажет кнопку "Authenticate" для сервера, о котором ещё не знает из
своего конфига.
...
**Если клиент -- Claude Desktop, `client-config.example.json` в
исходном виде не сработает для `planka` и `google-workspace`.**
Локальный `claude_desktop_config.json` в Claude Desktop поддерживает
только stdio-серверы (`command`/`args`) -- записи с `"type": "sse"` или
`"type": "http"` он молча отбрасывает при загрузке, и в списке серверов
может оказаться пусто, хотя сам JSON-файл синтаксически корректен и
`wordpress` (stdio) в нём есть. У Claude Desktop подключить такой сервер
через файл конфига нельзя вообще -- удалённые серверы там добавляются
только через UI (Settings -> Connectors -> "Add custom connector"), а
это соединение устанавливает облако Anthropic, а не компьютер человека,
поэтому там требуется публично доступный HTTPS-адрес. `planka-mcp` и
`google-workspace-mcp` нарочно опубликованы только на `127.0.0.1` (см.
комментарий в `docker-compose.yml` про `planka-mcp` -- у его SSE
эндпоинта нет проверки авторизации на уровне запроса, и выставлять его
в публичный интернет через ngrok/Cloudflare Tunnel нельзя), так что
путь через "Add custom connector" для них не подходит принципиально, а
не из-за версии клиента.

Рабочий путь для `planka` и `google-workspace` в Claude Desktop -- тот
же локальный мост `mcp-remote`, что и для по-настоящему старых клиентов:
он работает на компьютере человека, а не в облаке, и представляется
Claude Desktop обычным stdio-сервером, который файл конфига понимает.
Понадобится установленный Node.js (для `npx`). Замени в конфиге записи
`planka` и `google-workspace` на:
...
`wordpress` в конфиге не трогай -- он и так stdio, `mcp-remote` ему не
нужен.

У Claude Desktop нет слэш-команды `/mcp` (это специфика Claude Code) --
статус серверов и переподключение для Claude Desktop смотри в Settings
-> Developer.

---

## Шаг 8 -- OAuth-авторизация Google
...
отключён при OAuth 2.1). Конфиг из шага 7 уже должен быть записан и
клиент перезапущен/переподключён -- иначе сервер `google-workspace`
просто не появится в списке. Порядок действий зависит от клиента; для
Claude Code:
...
Если клиент -- Claude Desktop и `google-workspace` настроен через
`mcp-remote` (см. шаг 7): явной кнопки "Authenticate" может не быть --
`mcp-remote` сам откроет браузер с экраном согласия Google при первой
попытке клиента вызвать инструмент этого сервера. Если браузер не
открылся сам, попроси человека задать агенту любой вопрос про Google
Docs/Sheets/Drive (или переподключить сервер через Settings ->
Developer) -- это и есть тот "первый вызов", который запускает OAuth.

Если клиент -- что-то ещё, скажи человеку поискать аналогичное действие
"Authenticate"/"Connect"/"Sign in" рядом с сервером `google-workspace` --
сам сервер инициативно ссылку не покажет, инициатива со стороны клиента.
...
google-workspace-mcp`) и убедись, что шаг с "Authenticate" (или первый
вызов инструмента, для `mcp-remote`) был пройден до конца, а не просто
начат.
